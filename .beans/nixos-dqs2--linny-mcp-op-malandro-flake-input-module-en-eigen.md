...
updated_at: 2026-09-25T07:44:20Z
parent: nixos-m0vn
---
...
## Todo
- [ ] flake-input `linny-mcp.url = "github:linden-project/linny-mcp-server"` + `inputs.nixpkgs.follows`
- [ ] nieuwe module `modules/linny-mcp.nix`, geimporteerd in `hosts/malandro/configuration.nix`
- [ ] `imports = [ inputs.linny-mcp.nixosModules.linny-mcp ]` + `nixpkgs.overlays = [ inputs.linny-mcp.overlays.default ]`
- [ ] `services.linny-mcp`: `enable`, `listenAddress` (privé), `port` (vrije poort — check `PORTS.md`!),
...
- [ ] `systemd.tmpfiles.rules` voor corpus + state, eigendom van de service-user
- [ ] `PORTS.md` bijwerken met de gekozen poort
- [ ] `nix eval` op malandro: paden onder `/var/lib`, bind-adres privé
