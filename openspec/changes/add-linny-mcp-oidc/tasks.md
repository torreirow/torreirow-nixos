...
- [ ] 1.1 OIDC-client `claude-connector` in Authelia registreren (handmatig/tijdelijk, nog niet
...
- [ ] 1.2 Het bearer-schema toestaan op het `auth-request`-authz-endpoint en met `curl` bewijzen dat
      een `authelia_at_…` token 200 krijgt en een onzinnig token 401 — nog zonder linny-mcp erachter
- [ ] 1.3 `linny-mcp.toorren.net` tijdelijk publiek zetten met `auth_request` ervoor
...
- [ ] 2.1 De client uit 1.1 in `modules/authelia.nix` zetten, naast de Wallos-client
- [ ] 2.2 Client secret als agenix-secret (`authelia-claude-client-secret.age`); alleen de
      argon2-hash in de Authelia-config, het platte secret gaat naar Vaultwarden en de connector
- [ ] 2.3 Authz-endpoint met bearer-schema declaratief in `modules/authelia.nix`
- [ ] 2.4 Regressie: `linny.toorren.net` en de andere Authelia-vhosts nog steeds een redirect naar
      het portaal, niet een 401 — de authz-wijziging raakt ze allemaal

## 3. Nginx-kant in modules/linny-mcp.nix

- [ ] 3.1 Optie `oidc.enable` + `oidc.authzEndpoint`; `publicEndpoint` weer aan op malandro
- [ ] 3.2 `auth_request` vóór de proxy; `Host localhost` en de SSE-instellingen ongewijzigd laten
- [ ] 3.3 Statisch linny-mcp-token via agenix-snippet die nginx `include`t — **niet** als
      `proxy_set_header` in de nix-store
- [ ] 3.4 `/.well-known/oauth-protected-resource` serveren (statische JSON, wijst naar Authelia)
- [ ] 3.5 `WWW-Authenticate: Bearer resource_metadata="…"` op de 401 van `auth_request`
- [ ] 3.6 Leesgericht token genereren (`read:*`, eventueel `deny:taxonomy:…`) en in
      `secrets/linny-mcp-tokens.age` opnemen; het schrijfbare token van de tunnel blijft bestaan

## 4. Toetsen

- [ ] 4.1 `modules/linny-mcp_test.py` uitbreiden: `auth_request` aanwezig, well-known-location
